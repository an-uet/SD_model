...
# gene_pooling_layer = nn.AvgPool1d(kernel_size=4, stride=4).to(device)
...
def train(model, device, dataloader, optimizer, scheduler, epoch, iters, diversity_weight, epoch_loss, num_batches, current_lambda_cons):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        # gene_embeddings = gene_pooling_layer(tensor_gene).unsqueeze(1)  # (128, 2048) -> (128, 1, 512)
...
def run():
    ...
    # checkpoint = torch.load(f'{sd_path}/stable_diffusion_results/stable_diffusion_model_checkpoint_epoch_100.pth', map_location=device)
    # model.load_state_dict(checkpoint['model_state_dict'])
    # model.load_vae('vae_model.pth')
    model = load_vae_diffusion_model('/data/sunpengyu/Task_STsim/Model/STsim_Diffusion/vae_model.pth',
                                    in_channels=3,
                                    latent_dim=4,
                                    image_size=256,
    ...
    # val_dataloader = load_image(dataroot_test, image_size, batch_size, workers)

    # Freeze vae_params
    for param in model.vae.parameters():
        param.requires_grad = False
    # Make sure unet_params can be trained
