...
class SpatialTransformer(nn.Module):
    def __init__(self, hidden_dim, context_dim=2048, num_heads=4, self_attn=False, cross_attn=False):
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_dim, self_attn=False, cross_attn=False, num_heads=1, context_dim=2048):
        super().__init__()
...
class UNet_Transformer(nn.Module):
    def __init__(self, in_channels=3, time_dim=256, context_dim=2048):
        super().__init__()
    ...
    def forward(self, x, t, y):
        ...
        return x + initial_x  # Global residual connection


# Adding noise process: Start with the original image and gradually increase the noise until the final noisy image
class NoiseScheduler:
    def __init__(self, num_timesteps, device):
        self.device = device
        self.num_timesteps = num_timesteps
        self.betas = self.cosine_beta_schedule(num_timesteps).to(device) # use cosine schedule
